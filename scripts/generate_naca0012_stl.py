...
def write_stl(filename, upper, lower, span=0.1):
    ...
    for i in range(n - 1):
        triangles.append((
            ...
            (lower[i + 1][0], lower[i + 1][1], 0),
        ))
    # Back cap (z=span)
    for i in range(n - 1):
        triangles.append((
            (0.5, 0, span),
            (upper[i][0], upper[i][1], span),
            (upper[i + 1][0], upper[i + 1][1], span),
        ))
    for i in range(n - 1):
        triangles.append((
            (0.5, 0, span),
            (lower[i + 1][0], lower[i + 1][1], span),
            (lower[i][0], lower[i][1], span),
        ))
